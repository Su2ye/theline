...
| 优先级 | 模块 | 具体方案 |
|---|---|---|
| 高 | 线段端点 | 用两个人的头像（或首字母头像）替代当前圆点，线段变成「从你到对方」 |
| 高 | 线段色彩 | 从固定色相改为渐变——关系热络时暖色，疏远时冷色，不只是粗细变化 |
| 高 | 线段粒子 | 线周围飘浮微小的光点粒子，见面后几天内粒子最多，随时间消散 |
...
| 优先级 | 项目 | 说明 |
|---|---|---|
| 高 | 状态切换动画 | 线段从 warning → critical → disconnected 的切换目前是瞬间的，应该做 3-5 秒渐变过渡 |
| 高 | 见面瞬间 | 检测到见面时，线段短暂闪亮 + 轻微震动效果，给用户即时反馈 |
...
| 优先级 | 功能 | 说明 |
|---|---|---|
| 高 | 见面通知 | 见面检测成功时，本地弹一条通知 + 震动（不需要服务器） |
| 高 | 照片压缩 | 当前 base64 原图存储，应压缩到 800px 宽 / 200KB 以内再存 IndexedDB |
